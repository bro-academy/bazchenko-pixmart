...
async function addDimensionsToImages(html, assetDir) {
  ...
  while ((match = tagRegex.exec(html)) !== null) {
    ...
    const srcMatch = tag.match(/\s(src|srcset)=["']([^"']+\.(png|jpe?g|webp|avif)(\?[^"']*)?)["']/i);
    if (!srcMatch) {
      result += tag;
...
function collectNeededImages(htmlDir) {
  ...
  for (const file of htmlFiles) {
    ...
    const regex =
      /src=["']\/assets\/([^"']+\.(png|jpe?g))(?:\?format=(webp|avif))?/gi;
    let match;
    while ((match = regex.exec(html))) {
      const base = match[1].replace(/-[A-Za-z0-9]{8,}(?=\.(png|jpe?g)$)/, '');
      const format = match[3];
      if (!needed[base]) needed[base] = new Set();
      if (format) {
        needed[base].add(format);
      } else {
        needed[base].add(path.extname(base).slice(1));
      }
    }
  }
  return needed;
...
function collectNeededImagesFromCSS(assetDir) {
  ...
  for (const file of cssFiles) {
    ...
    const regex = /url\(["']?(\/assets\/([^"')]+\.(png|jpe?g)))(?:\?format=(webp|avif))?["']?\)/gi;
    let match;
    while ((match = regex.exec(css))) {
      const base = match[2].replace(/-[A-Za-z0-9]{8,}(?=\.(png|jpe?g)$)/, '');
      const ext = match[3];
  ...
  const walk = (dir) => {
    for (const file of fs.readdirSync(dir)) {
      ...
      if (fs.statSync(abs).isDirectory()) {
        ...
        if (
          file.replace(/-[A-Za-z0-9]{8,}(?=\.(png|jpe?g)$)/, '') === baseName
        ) {
    ...
    async closeBundle() {
      ...
      for (const [base, formatSet] of Object.entries(needed)) {
        ...
        if (formatSet.has(ext)) {
          ...
          const tmpPath = path.join(os.tmpdir(), `opt-${path.basename(hashed)}`);
          if (ext === 'png') {
        ...
        for (const format of formatSet) {
          ...
          const outPath = filePath.replace(/\.(png|jpe?g)$/i, `.${format}`);
      ...
      for (const file of htmlFiles) {
        ...
        html = html.replace(
          /((src|srcset)=["'])([^"']+\.(png|jpe?g))(\?format=(webp|avif))(["'])/gi,
          (match, prefix, attr, url, ext, query, format, suffix) => {
            const newUrl = url.replace(/\.(png|jpe?g)$/i, `.${format}`);
            return `${prefix}${newUrl}${suffix}`;
          },
        );
        html = html.replace(
          /((src|srcset)=["'])([^"']+\.(png|jpe?g))(\?optimize)(["'])/gi,
      ...
      for (const file of cssFiles) {
        ...
        css = css.replace(
          /url\((["']?)(\/assets\/[^"')]+\.(png|jpe?g))\?format=(webp|avif)\1\)/gi,
          (match, quote, url, ext, format) =>
            `url(${quote}${url.replace(/\.(png|jpe?g)$/i, `.${format}`)}${quote})`
        );
